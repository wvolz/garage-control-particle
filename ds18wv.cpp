...
DS18WV::DS18WV(uint16_t pin, bool parasitic)
  :
  _wire{pin},
...
bool DS18WV::read(uint8_t addr[8]) {
...
float DS18WV::celsius() {
  return _celsius;
}

float DS18WV::fahrenheit() {
  return _celsius * 1.8 + 32.0;
}

void DS18WV::addr(uint8_t dest[8]) {
  memcpy(dest, _addr, sizeof(_addr));
}

void DS18WV::data(uint8_t data[9]) {
  memcpy(data, _data, sizeof(_data));
}

DS18WVType DS18WV::type() {
  return _type;
}

bool DS18WV::searchDone() {
  return _searchDone;
}

bool DS18WV::crcError() {
  return _crcError;
}

void DS18WV::setConversionTime(uint16_t ms) {
  _conversionTime = ms;
}